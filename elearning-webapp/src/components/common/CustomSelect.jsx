...
const CustomSelect = ({ 
  ...
  const filteredOptions = React.useMemo(() => {
    ...
    return options.filter(opt => 
      ...
      (typeof opt.value === 'string' && opt.value.toLowerCase().includes(query))
    );
  ...
  const dropdownList = (
    <div 
      className="custom-select-dropdown mt-2 max-h-64 overflow-y-auto overflow-x-hidden rounded-2xl border border-slate-100 bg-white shadow-[0_10px_40px_-10px_rgba(15,23,42,0.15)] animate-in fade-in zoom-in-95 duration-200"
      style={{
        position: 'fixed',
        top: `${coords.top}px`,
        left: `${coords.left}px`,
    ...
    >
      {searchable && (
        <div className="p-2 border-b border-slate-100 sticky top-0 bg-white z-10">
          <input
            type="text"
            className="form-input text-xs w-full py-1.5 px-3 rounded-xl border-slate-200 bg-slate-50 focus:bg-white transition-colors"
